...
public class BoardService {
    ...
    public Board 수정폼게시글정보(int id, int sessionUserId) {
        ...
        if (sessionUserId != board.getUser().getId())
            throw new Exception403("수정할 권한이 없어요");
        return board;
    }
    ...
    public void 게시물수정(int id, String title, String content, int sessionUserId) {
        ...
        if (sessionUserId != board.getUser().getId())
            throw new Exception403("게시글을 수정할 권한이 없어요");
    ...
    public void 게시글삭제(int id, int sessionUserId) {
        Board board = boardRepository.findById(id) // 영속화
                .orElseThrow(() -> new Exception404("삭제 할 게시글을 찾을 수 없어요"));

        if (sessionUserId != board.getUser().getId())
            throw new Exception403("삭제할 권한이 없어요");

        board.getReplies().forEach(r -> { // null로 상태 변경
            r.setBoard(null);
        });
